...
TIER_CONFIGS = {
    1: {
        "tier_name": "Minimal",
        "recommended_model": "Qwen-3.6-27B-MoE",
        "gpu_layers": 0,
        "context_size": 2048,
        "threads": 4,
        "batch_size": 256,
    ...
    4: {
        "tier_name": "High",
        "recommended_model": "Qwen-3.6-35B-MoE",
        "gpu_layers": 35,
        "context_size": 8192,
        "threads": 8,
        "batch_size": 512,
                ...
                for line in result.stdout.split("\n"):
                    ...
                    if "intel" in line_lower and ("iris" in line_lower or "uhd" in line_lower or "hd graphics" in line_lower):
                        ...
                        and (
                            ...
                            or "amd radeon(tm) graphics" in line_lower
                        )
...
def _score_cpu_arch(features: List[str]) -> int:
...
def _score_cores(physical_cores: int) -> int:
...
def _score_ram_total(total_gb: float) -> int:
    """Score total RAM (1–5)."""
    if total_gb >= 64:
        return 5
    if total_gb >= 32:
...
def _score_ram_available(available_gb: float) -> int:
...
def _score_gpu_vram(gpu: Dict[str, Any]) -> int:
...
def _score_igpu(igpu: Dict[str, Any]) -> int:
...
def _score_memory_bandwidth(ddr_type: str) -> int:
...
def _score_vendor_fit(vendor: str, has_dgpu: bool, igpu: Dict[str, Any]) -> int:
...
def rate_system(cpu: Dict[str, Any], ram: Dict[str, Any], gpu: Dict[str, Any], igpu: Dict[str, Any]) -> Tuple[int, Dict[str, int], float]:
    """
...
def recommend_backend(cpu: Dict[str, Any], gpu: Dict[str, Any], igpu: Dict[str, Any]) -> str:
...
def get_recommended_config(tier: int, gpu: Dict[str, Any], igpu: Dict[str, Any], backend_hint: str, ram: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
def print_specs(specs: Dict[str, Any]) -> None:
