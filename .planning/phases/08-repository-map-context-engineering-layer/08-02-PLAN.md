---
id: "08-02"
title: "Tree-sitter Symbol Indexer & SQLite Cache"
phase: 8
wave: 1
depends_on: []
files_modified:
...
# Plan 08-02: Tree-sitter Symbol Indexer

<objective>
Implement the full Tree-sitter symbol indexer in `src/context/indexer.rs`. This module:
1. Parses Rust source files using `tree-sitter` + `tree-sitter-rust`
2. Extracts symbols (functions, structs, enums, traits, impls, mods, use declarations)
3. Builds a petgraph `StableGraph` of symbol relationships (import edges, impl edges)
4. Caches symbol data in SQLite (`symbol_cache` table) with SHA-256-based incremental invalidation
5. Provides `find_symbol` for O(log n) SQLite lookups and `list_all_symbols` for PageRank input

This runs in Wave 1 in parallel with Plan 08-01 (different files, no dependency).
</objective>

<must_haves>
- `Indexer::new(conn, workspace_root)` builds or restores index from SQLite cache
- `Indexer::index_workspace()` walks `workspace_root` with `ignore::WalkBuilder` (gitignore-aware), only processes `.rs` files, skips unchanged files (SHA-256 match)
- `Indexer::find_symbol(name)` returns `Vec<SymbolNode>` matching that name within <3 seconds on the helix-cli workspace
- `SymbolNode` carries: `file_path`, `symbol_name`, `symbol_kind`, `signature` (body-elided), `line_start`, `line_end`, `doc_comment`
- SQLite `symbol_cache` table created if not exists with schema from research
- Incremental update: files with unchanged SHA-256 are skipped
- All code compiles with `cargo build -p agent-rs`
</must_haves>

---

## Task 08-02-01: Define `SymbolNode` and SQLite Schema

<read_first>
- `agent-rs/src/context/indexer.rs` — current stub (`pub struct Indexer;`)
- `agent-rs/Cargo.toml` — confirm `rusqlite`, `sha2`, `hex`, `ignore`, `serde`, `serde_json` are present
- `agent-rs/src/context/mod.rs` — `ContextResult` type for reference on field naming conventions
</read_first>

<action>
Replace `agent-rs/src/context/indexer.rs` stub with:
...
<acceptance_criteria>
- `agent-rs/src/context/indexer.rs` contains `pub struct Indexer`
- `agent-rs/src/context/indexer.rs` contains `pub fn parse_rust_file`
- `agent-rs/src/context/indexer.rs` contains `pub fn find_symbol`
- `agent-rs/src/context/indexer.rs` contains `const SCHEMA: &'static str` with `CREATE TABLE IF NOT EXISTS symbol_cache`
- `cargo test context::indexer -p agent-rs -- --nocapture 2>&1 | Select-String "FAILED"` returns no results
- `cargo test context::indexer::benchmark_symbol_lookup_under_3s -p agent-rs --release 2>&1` passes without panic
- `cargo test context::indexer::test_parse_partial_code_no_panic -p agent-rs 2>&1 | Select-String "FAILED"` returns no results
</acceptance_criteria>
