---
id: "08-03"
title: "Durable Agent Memory Layer (SQLite FTS5)"
phase: 8
wave: 2
...
# Plan 08-03: Durable Agent Memory Layer

<objective>
Implement the full `src/context/memory.rs` module — the durable agent memory layer backed by SQLite FTS5. This module stores goals, constraints, decisions, failed attempts, and the edit ledger across context compaction cycles (LLM context window resets). Uses the `rusqlite` dependency already in the project.

Wave 2: depends on Plans 08-01 (types established) and 08-02 (SQLite connection pattern established).
</objective>

<must_haves>
- `MemoryEngine::new(conn)` creates all required tables and FTS5 virtual table
- `MemoryEngine::new_session(goal)` creates a new session row and returns the UUID session ID
- `MemoryEngine::record(session_id, kind, content)` inserts a memory entry and syncs FTS5
- `MemoryEngine::load_session(session_id)` returns all memory entries for a session ordered by importance DESC
- `MemoryEngine::list_active_sessions()` returns all active sessions
- `MemoryEngine::record_edit(session_id, file_path, edit_kind, diff_patch)` appends to edit_ledger
- `MemoryEngine::resume_session(session_id)` returns a compact formatted string for LLM context re-injection
- `MemoryEngine::search_memory(query)` uses FTS5 to find relevant memory entries
- Session state survives drop and re-open (the compaction survival test must pass)
- WAL mode enabled
</must_haves>

---

## Task 08-03-01: Implement Full `memory.rs`

<read_first>
- `agent-rs/src/context/memory.rs` — current stub (`pub struct MemoryEngine;`)
- `agent-rs/src/context/indexer.rs` — review SQLite connection pattern, WAL mode, transaction usage
- `agent-rs/Cargo.toml` — confirm `rusqlite`, `serde`, `serde_json`, `chrono` are present
</read_first>

<action>
Replace the stub `agent-rs/src/context/memory.rs` with the full implementation:
...
<acceptance_criteria>
- `agent-rs/src/context/memory.rs` contains `pub struct MemoryEngine`
- `agent-rs/src/context/memory.rs` contains `pub enum MemoryKind`
- `agent-rs/src/context/memory.rs` contains `SCHEMA` with `CREATE VIRTUAL TABLE IF NOT EXISTS memory_fts USING fts5`
- `agent-rs/src/context/memory.rs` contains `pub fn resume_session`
- `cargo test context::memory -p agent-rs -- --nocapture 2>&1 | Select-String "FAILED"` returns no results
- `cargo test context::memory::test_session_survives_compaction -p agent-rs 2>&1` passes
- `cargo test context::memory::test_fts5_search -p agent-rs 2>&1 | Select-String "FAILED"` returns no results
</acceptance_criteria>
